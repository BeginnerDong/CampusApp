	...
	<view>
		<view class="joinActive mglr4" v-if="mainData.length>0">
			...
			@click="Router.navigateTo({route:{path:'/pages/activityDetail/activityDetail?id='+$event.currentTarget.dataset.id}})">
				...
				<view class="quanCard fs15" style="position: relative;">
					<image :src="item.title?'../../static/images/canyuhuodong-img.png':'../../static/images/canyuhuodong-img.png'" 
					style="position: absolute;width: 100%;height: 100%;top: 0;left: 0;z-index:0"></image>
					<view style="z-index: 999;">
						<view class="center fs22 ftw quanTit pdb15" style="position: absolute;left: 50%;">入场券</view>
						<view style="position: absolute;top:40%">时间：{{item.start_time}}</view>
						<view class="pdt10 avoidOverflow" style="position: absolute;top:52%;width: 65%;">地点：{{item.address}}</view>
					</view>
...
<style>
	...
	.quanCard{width: 100%;height: 316rpx;padding: 50rpx 30rpx 30rpx 210rpx;box-sizing: border-box;}
