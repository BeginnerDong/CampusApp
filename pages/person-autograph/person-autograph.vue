	...
	<view>
		...
		<view class="pdlr4 whiteBj" style="z-index: 2;padding-top: 44px;">
			<view class="flexRowBetween pdtb10">
				...
				@click="$Utils.stopMultiClick(userInfoUpdate)">确定</view>
			</view>
		</view>
		<view class="pdlr4 mgt15">
			<view class="pdb10" style="border-bottom: 1px solid #ddd;">
				<input type="text" v-model="submitData.signature" placeholder="请输入签名" placeholder-class="placeholder">
			</view>
			<view class="pdtb10 fs12 color9">签名可以让你的朋友容易记住你。</view>
		</view>
...
<script>
	export default {
		...
		onLoad(options) {
			const self = this;
			self.submitData.signature = options.name
		...
		methods: {
			...
			userInfoUpdate() {
				...
				if(self.submitData.signature==''){
					uni.setStorageSync('canClick', true);
					self.$Utils.showToast('签名不能为空', 'none', 1000)
					return
				};
				const postData = {};
