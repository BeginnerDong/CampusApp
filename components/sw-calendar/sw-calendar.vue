		...
		<view class="date-wrap">
			...
			<view class="date-item-box item-box">
				<view class="item date-item" 
					...
					:key="index">
					<view class="dian">{{item.date}}</view>
	...
	mounted () {
		...
		nodes.fields({
		  size: true,
			...
			// height:${data.width}px;line-height:${data.width}px;
			this.itemStyle = `margin: 10upx calc((100% - ${data.width*7}px) / 14);`
		}).exec();
...
<style>
	...
	.item-box{display: flex;color: #777;flex-wrap: wrap;font-size: 28upx;justify-content: flex-start;}
	.item{width: calc(100% / 12); margin: 10upx auto;color: #222;}
	...
	.date-item-box{padding: 0 2%;font-size: 24upx;}
